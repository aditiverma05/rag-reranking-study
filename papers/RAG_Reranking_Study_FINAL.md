...
**RAG evaluation.** Es et al. (2023) introduce RAGAS, a reference-free evaluation framework for RAG pipelines that decomposes evaluation into retrieval quality (context precision/recall), faithfulness (whether the generated answer is supported by retrieved context), and answer relevancy. Our study adopts a similar decomposition — separating retrieval-quality metrics (Hit@k) from answer-quality metrics (token-level F1) — but relies on gold-passage matching rather than LLM-judged scores for the retrieval side, trading some of RAGAS's flexibility for metrics that do not depend on an additional judge model and its associated cost and variance.
